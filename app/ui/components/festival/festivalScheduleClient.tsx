...
export default function FestivalScheduleClient({ events, dates, venues, tags }: Props) {
  ...
  const sortedDates = useMemo(() => {
    return [...dates].sort((a, b) => {
      ...
      const parseDate = (slug: string) => {
        ...
        const month = months.indexOf(parts[0].charAt(0).toUpperCase() + parts[0].slice(1))
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(event => {
      if (activeDate && !event.festivalDates?.nodes.some(d => d.slug === activeDate?.slug)) return false
      if (activeVenue && !event.festivalVenues?.nodes.some(v => v.slug === activeVenue)) return false
      if (activeTags.length > 0 && !activeTags.every(t => event.eventiveTags?.nodes.some(et => et.slug === t))) return false
  ...
  const hasFilters = activeVenue || activeTags.length > 0 || showInPerson || showVirtual
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.topNav}>
        <Link href={`/festival/festival-2025/schedule`} className={`${styles.topNavItem} ${styles.topNavItemActive}`}>
          Schedule
        </Link>
        <Link href={`/festival/festival-2025/film-guide`} className={styles.topNavItem}>
          Films A–Z
        </Link>
        <Link href={`/festival/festival-2025/event-guide`} className={styles.topNavItem}>
          Talks & Events
        </Link>
        <Link href={`/festival/festival-2025/my-schedule`} className={styles.topNavItem}>
          My Schedule
        </Link>
      </div>
      ...
      <div className={styles.dateNav}>
        ...
        <div className={styles.dateNavCenter}>
          ...
          <button
            className={styles.dateArrow}
            onClick={() => setActiveDateIndex(i => Math.min(sortedDates.length - 1, i + 1))}
        ...
        <div className={styles.dateNavRight}>
          ...
          <div className={styles.personageToggle}>
            <button
              ...
              onClick={() => { setShowInPerson(true); setShowVirtual(false) }}
            >
              IN PERSON
            </button>
            <button
              className={`${styles.personageBtn} ${showVirtual && !showInPerson ? styles.personageBtnActive : ''}`}
              onClick={() => { setShowVirtual(true); setShowInPerson(false) }}
            >
              VIRTUAL
            </button>
            {(showInPerson || showVirtual) && (
              <button
                className={styles.personageBtn}
                onClick={() => { setShowInPerson(false); setShowVirtual(false) }}
              >
                ALL
              </button>
            )}
        ...
        <div className={styles.filtersPanel}>
          <div className={styles.filterGroup}>
            ...
            >
              ...
              {venues.map(venue => (
                <option key={venue.slug} value={venue.slug}>{venue.name}</option>
              ))}
            </select>
          </div>
          <div className={styles.filterGroup}>
        ...
        {filteredEvents.length === 0 ? (
          ...
          filteredEvents.map((event, i) => {
            ...
            const isFilm = event.eventiveTags?.nodes.some(t =>
              ['feature-narrative','feature-documentary','short-narrative','short-documentary','experimental','shorts-program','shorts-programs'].includes(t.slug)
            )
            ...
            return (
              <div key={i} className={styles.eventRow}>
                <div className={styles.eventLeft}>
                  ...
                  <div className={styles.eventTags}>
                    ...
                    {event.premiereStatuses?.nodes.map((s, j) => (
                      <span key={j} className={styles.eventTag}>({s.name.toUpperCase()})</span>
                    ))}
                    {event.festivalAwards?.nodes.map((a, j) => (
                      <span key={j} className={styles.eventTag}>({a.name.toUpperCase()})</span>
                    ))}
                  ...
                  <h2 className={styles.eventTitle}>
                    <Link href={`/festival/events/${event.slug}`} className={styles.eventTitleLink}>
                      {event.title}
                    </Link>
                  </h2>
                  ...
                  <div className={styles.eventMeta}>
                    ...
                    {acf?.startTime && acf.timezone && (
                      <span>
                        {event.festivalDates?.nodes[0]?.name}, {formatTime(acf.startTime, acf.timezone)} {acf.timezone === 'America/New_York' ? 'EST' : ''}
                      </span>
                    )}
                  ...
                  <div className={styles.eventActions}>
                    <Link href={`/festival/events/${event.slug}`} className={styles.actionBtn}>
                      READ MORE
                    </Link>
                    {acf?.ticketsAvailable && !acf.hideTicketsButton && acf.eventiveId && (
                      <div
                ...
                {event.featuredImage?.node?.sourceUrl && (
                  <Link href={`/festival/events/${event.slug}`} className={styles.eventImageWrapper}>
                    <Image
